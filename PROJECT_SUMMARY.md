...
> **更新日期:** 2026-06-19
> **当前版本:** v11.0.1
> **项目状态:** v11.0 全面视觉统一与交互优化已完成；基于 v10.0 新增全局色彩统一、排序序号标识、字典树动画重设计、页面视觉与交互体验提升、AVL 树数据结构、本地打开兼容修复
> **验证基线:** Lint 0 errors / Build 成功 / 3042 单元测试通过（188 文件） / TypeScript strict 模式 / Bundle 预算符合
...
| React Router | 7.15 | 路由（协议感知 Router，14 页 React.lazy 代码分割） |
| Vitest | 4.1 | 单元测试（3042 tests / 188 文件） |
...
```
Entry (main.tsx → App.tsx) → Pages (14) → Components (33) → Hooks (22) → Visualizers (11) → Algorithms/Utils
```

- **状态管理:** 无 Redux/Zustand，每个数据结构独立 `use*State` Hook（12 个），内部统一使用 `useHistory`（useRef 实现的撤销/重做栈，最多 20 步）
...
- **数据持久化:** 12 种数据结构通过 localStorage 自动保存/恢复
- **国际化:** 自研轻量 i18n（中文+英文）
...
| 树形结构 | TreePage, AvlTreePage, HeapPage, TriePage | 二叉搜索树、AVL 树、堆、字典树 |
...
| 单元测试 | ✅ | 3042 tests / 188 文件（v11.0 coverage） |
...
| 优先级 | 任务 | 说明 |
|--------|------|------|
| ✅ 已完成 | 动画性能优化与大数据降级 | Phase U1 已完成：统一性能配置、transform/opacity 动画迁移、FPS 自动降级、渲染耗时观测 |
