...
describe('useArrayState', () => {
  ...
  it('应该支持搜索元素', () => {
    ...
    let index = -1
    act(() => { index = result.current.search(12) })
    expect(index).toBe(2)
  })
  ...
  describe('insert 边界条件', () => {
    it('数组满时插入应该失败', () => {
      ...
      let success = true
      act(() => { success = result.current.insert(99, 0) })
    ...
    it('负索引插入应该失败', () => {
      const { result } = renderHook(() => useArrayState())
      let success = true
      act(() => { success = result.current.insert(1, -1) })
      expect(success).toBe(false)
    })
    ...
    it('超范围索引插入应该失败', () => {
      const { result } = renderHook(() => useArrayState())
      let success = true
      act(() => { success = result.current.insert(1, 100) })
      expect(success).toBe(false)
    })
    ...
    it('无效输入应该失败', () => {
      const { result } = renderHook(() => useArrayState())
      let success = true
      act(() => { success = result.current.insert('abc', 0) })
      expect(success).toBe(false)
    })
    ...
    it('边界值 0 应该失败', () => {
      const { result } = renderHook(() => useArrayState())
      let success = true
      act(() => { success = result.current.insert(0, 0) })
      expect(success).toBe(false)
    })
    ...
    it('边界值 100 应该失败', () => {
      const { result } = renderHook(() => useArrayState())
      let success = true
      act(() => { success = result.current.insert(100, 0) })
      expect(success).toBe(false)
    })
  ...
  describe('remove 边界条件', () => {
    it('负索引删除应该返回 null', () => {
      ...
      act(() => { value = result.current.remove(-1) })
    ...
    it('超范围索引删除应该返回 null', () => {
      const { result } = renderHook(() => useArrayState())
      let value: number | null = 0
      act(() => { value = result.current.remove(100) })
      expect(value).toBeNull()
    })
    ...
    it('空数组删除应该返回 null', () => {
      ...
      act(() => { result.current.loadData([]) })
      let value: number | null = 0
      act(() => { value = result.current.remove(0) })
      expect(value).toBeNull()
    })
  })
  ...
  describe('search 边界条件', () => {
    ...
    it('空数组搜索应该返回 -1', () => {
      ...
      act(() => { result.current.loadData([]) })
      let index = 0
      act(() => { index = result.current.search(1) })
      expect(index).toBe(-1)
    })
  })
  ...
  describe('searchAll', () => {
    it('应该返回所有匹配项的索引', () => {
      ...
      let indices: number[] = []
      act(() => { indices = result.current.searchAll(5) })
    ...
    it('无匹配时应该返回空数组', () => {
      const { result } = renderHook(() => useArrayState())
      let indices: number[] = []
      act(() => { indices = result.current.searchAll(999) })
    ...
    it('空数组应该返回空数组', () => {
      ...
      act(() => { result.current.loadData([]) })
      let indices: number[] = []
      act(() => { indices = result.current.searchAll(1) })
      expect(indices).toEqual([])
    })
    ...
    it('单匹配应该返回单元素数组', () => {
      const { result } = renderHook(() => useArrayState())
      let indices: number[] = []
      act(() => { indices = result.current.searchAll(12) })
  ...
  describe('binarySearch', () => {
    it('有序数组中查找存在的值应该返回正确索引', () => {
      ...
      let index = -2
      act(() => { index = result.current.binarySearch(7) })
      expect(index).toBe(3)
    })
    ...
    it('有序数组中查找不存在的值应该返回 -1', () => {
      ...
      let index = -2
      act(() => { index = result.current.binarySearch(6) })
      expect(index).toBe(-1)
    })
    ...
    it('无序数组应该返回 -1（前置条件失败）', () => {
      ...
      // 默认数据 [8, 3, 12, 5, 9] 无序
      let index = -2
      act(() => { index = result.current.binarySearch(8) })
      expect(index).toBe(-1)
    })
    ...
    it('空数组应该返回 -1', () => {
      ...
      act(() => { result.current.loadData([]) })
      let index = -2
      act(() => { index = result.current.binarySearch(1) })
      expect(index).toBe(-1)
    })
    ...
    it('单元素有序数组查找命中应该返回 0', () => {
      ...
      let index = -2
      act(() => { index = result.current.binarySearch(42) })
      expect(index).toBe(0)
    })
    ...
    it('查找首元素应该返回 0', () => {
      ...
      let index = -2
      act(() => { index = result.current.binarySearch(2) })
      expect(index).toBe(0)
    })
    ...
    it('查找末元素应该返回最后一个索引', () => {
      ...
      act(() => { result.current.loadData([2, 4, 6, 8, 10]) })
      let index = -2
      act(() => { index = result.current.binarySearch(10) })
      expect(index).toBe(4)
    })
    ...
    it('允许重复值的有序数组应返回其中一个匹配索引', () => {
      ...
      let index = -2
      act(() => { index = result.current.binarySearch(3) })
