...
describe('PerformanceChart', () => {
  ...
  describe('渲染', () => {
    it('应该渲染 PerformanceChart 组件', () => {
      const { container } = render(<PerformanceChart results={sampleResults} />)
  ...
  describe('空数据', () => {
    it('应该渲染空对象 results', () => {
      const { container } = render(<PerformanceChart results={{}} />)
      const svg = container.querySelector('svg')
      expect(svg).toBeInTheDocument()
    })
  ...
  describe('D3 调用验证', () => {
    ...
    it('应该调用 append 创建 SVG 元素', () => {
      render(<PerformanceChart results={sampleResults} />)
