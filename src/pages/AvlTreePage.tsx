...
export default function AvlTreePage() {
  ...
  }, [isAnimating, isDelaying, flattenedData, setIsAnimating, getAnimationContext, svgRef, t])
  ...
  return (
    <div className="flex flex-col min-h-dvh bg-paper dark:bg-dark-paper grain">
      ...
      <OperationBar>
        ...
        <OperationButton variant="primary" onClick={handleInsert} disabled={isAnimating || isDelaying} isBusy={isAnimating || isDelaying}>{t('avlTree.insert')}</OperationButton>
        ...
        <OperationButton variant="warning" onClick={handleSearch} disabled={isAnimating || isDelaying} isBusy={isAnimating || isDelaying}>{t('avlTree.search')}</OperationButton>
        <OperationButton variant="primary" onClick={() => handleTraversal(preorder)} disabled={isAnimating || isDelaying} isBusy={isAnimating || isDelaying} popAnimation>{t('avlTree.preorder')}</OperationButton>
        <OperationButton variant="primary" onClick={() => handleTraversal(inorder)} disabled={isAnimating || isDelaying} isBusy={isAnimating || isDelaying} popAnimation>{t('avlTree.inorder')}</OperationButton>
        ...
        <OperationGroup label={t('common.more')}>
          <OperationButton variant="primary" onClick={() => handleTraversal(postorder)} disabled={isAnimating || isDelaying} isBusy={isAnimating || isDelaying} popAnimation>{t('avlTree.postorder')}</OperationButton>
          <OperationButton variant="primary" onClick={() => handleTraversal(levelorder)} disabled={isAnimating || isDelaying} isBusy={isAnimating || isDelaying} popAnimation>{t('avlTree.levelorder')}</OperationButton>
          <UndoPreviewButton
            variant="secondary"
            onClick={undo}
      ...
      <Visualizer
        ...
        overlay={isDelaying ? <AnimationDelayIndicator /> : undefined}
      />
      {nodeCount === 0 && (
